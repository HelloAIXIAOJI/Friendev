...
fn apply_edits(content: &mut String, args: &FileReplaceArgs) -> (usize, Vec<(usize, String)>) {
    ...
    for (edit_idx, edit) in args.edits.iter().enumerate() {
        if edit.regex {
            apply_regex_edit(content, edit, edit_idx, &mut replacements_made, &mut failed_edits);
        } else {
            apply_string_edit(content, edit, edit_idx, &mut replacements_made, &mut failed_edits);
        }
    }
